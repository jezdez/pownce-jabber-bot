...
def pownce_post(sender, instance, signal, *args, **kwargs):
    ...
    if pownce_login(username, password) and not note.is_sent:
        ...
        try:
            ...
            mail_subject = "Posting of %s by user %s was not successful." % (note.type, username)
            mail_body = mail_subject
            if hasattr(e, 'code'):
                mail_body += '\nWe failed with error code - %s.' % e.code
            elif hasattr(e, 'reason'):
            ...
            dispatcher.send(signal=bot_log, text="succesfully posted %s by %s." % (note.type, username))
            dispatcher.send(signal=post_message_sent, user=xmpp.JID(note.user.jid), text="I succesfully posted your %s." % note.type)
        try:
            note.is_sent = True
            note.save()
        except:
...
class Note(models.Model):
    ...
    to = models.CharField(_("to"), max_length=100, default=DEFAULT_TO)
    created = models.DateTimeField(_("created"), default=datetime.now)
    is_sent = models.NullBooleanField(_("is sent"), blank=True, null=True)
    
    class Admin:
        list_display = ('user', 'created', 'type', 'is_sent')
        list_filter = ('is_sent', 'type', 'to')
